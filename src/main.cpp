...
#if defined(NDEBUG)
# error "Tribe cannot be compiled without assertions."
#endif
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, int nHeight)
{
    ...
    if (nHeight >= 109356 // Checkpoint-2
        ...
        std::string aBlockedTxs[] = {
            ...
            "44813cf858d017d445ffe5718b207ae328be24fd87ef8557e0a792a3a66a6d94",
            "4cac4d2fc3c0b2361af732c39b285f67a974377598abe8d6e547c646e468711f",
            "79b0829be155367c175d279d570f63f98d83f393d7c6e3fab01fbcec4e914864",
            "67f2f12a97e037fa620bfa7c728209d939af821396d17ec0f1c9f3c151468a33"
        };
        std::set<std::string> sBlockedTxs(aBlockedTxs, aBlockedTxs + sizeof(aBlockedTxs) / sizeof(aBlockedTxs[0]));
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (sBlockedTxs.count(txin.prevout.hash.GetHex()))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-inputs-blocked");
        }
    }
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
    ...
    if (!CheckTransaction(tx, state, chainActive.Height()))
        return false;
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams)
{
    ...
    LogPrint("subsidy", "GetBlockSubsidy:: Spork: SPORK_14_REWARD_V2_UPDATE=%u\n", GetSporkValue(SPORK_14_REWARD_V2_UPDATE));
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int p30_period = GetSporkValue(SPORK_14_REWARD_V2_UPDATE);
    int p40_period = GetSporkValue(SPORK_14_REWARD_V2_UPDATE) + (1 * nIncreasePeriod);
    int p50_period = GetSporkValue(SPORK_14_REWARD_V2_UPDATE) + (2 * nIncreasePeriod);
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                while (!masternodeSync.IsSporksSynced())
                    MilliSleep(10);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Tribe Related Inventory Messages
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                LogPrint("net", "Large reorg, won't direct fetch to %s (%d)\n",
                        ...
                        pindexLast->nHeight);
            } else if (masternodeSync.IsSporksSynced()) {
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        //
        vector<CInv> vGetData;
        if (!pto->fDisconnect && masternodeSync.IsSporksSynced() && !pto->fClient && (fFetch || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
